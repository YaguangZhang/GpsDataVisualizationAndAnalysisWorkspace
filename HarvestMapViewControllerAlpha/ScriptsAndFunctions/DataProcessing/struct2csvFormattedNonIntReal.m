function struct2csvFormattedNonIntReal(s,fn, formatSpec)
% Modified by Yaguang Zhang 11/16/2017
%
% STRUCT2CSV(s,fn)
%
...
%
% Written by James Slegers, james.slegers_at_gmail.com Covered by the BSD
% License
%
    ...
    for ii = 1:n
        ...
        for jj = 1:m
            ...
            if sz(jj,1)<ii
                ...
                if isreal(c) && (~all(rem(c,1)==0))
                    for kk = 1:sz(jj,2)
                        str = [str,num2str(c(ii,kk), formatSpec),','];
                    end
                elseif isnumeric(c)
                    for kk = 1:sz(jj,2)
