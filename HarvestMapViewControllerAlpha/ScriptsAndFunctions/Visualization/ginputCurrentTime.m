function [out1,out2,out3] = ginputCurrentTime(arg1, hAxis, hCurrentTime, hText)
%GINPUTCURRENTTIME Modified graphical input from mouse.
%   
% Used for get mouse input as well update the current timeline figure
% accordingly.
%
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    while how_many ~= 0
        ...
        if(ptr_fig == fig)
            ...
            % Update currentTime line and text. - Zyg
            set(hCurrentTime, 'XData', [pt(1,1),pt(1,1)]);
            yLim = get(hAxis,'ylim');
            textPosition = [pt(1,1),(yLim(2)+yLim(1))/2, 0];
            set(hText, 'Position', textPosition, 'String', num2str(pt(1,1)));
