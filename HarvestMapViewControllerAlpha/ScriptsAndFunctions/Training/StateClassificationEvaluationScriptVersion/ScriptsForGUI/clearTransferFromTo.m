function handles = clearTransferFromTo(idxFrom, idxTo, timeStart, timeEnd, handles)
%CLEARTRANSFERFROMTO Clear the states.
%
% Inputs:
%   - idxFrom, idxTo
...
if idxFrom ~=0 % Not harvesting.
    ...
    handles.statesRef{idxFrom}(indicesToSetLoadFrom,2) = nan(length(indicesToSetLoadFrom),1);
end
...
if ~isinf(idxTo) % Not dumping to the factory.
    ...
    handles.statesRef{idxTo}(indicesToSetDumpTo,1) = nan(length(indicesToSetDumpTo),1);
end
