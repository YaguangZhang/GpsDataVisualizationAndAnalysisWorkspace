%GENMOVIEBYGPSTIMEFORSTATEREF Generate illustration movies according to the
%specified GPS time range, with vehicle states drawn according to
%statesRef.
%
% Revised from genMovieByGpsTimeForStateRef.m.
%
% Yaguang Zhang, Purdue, 08/18/2016
...
% Overwrite statesByDist by statesRef so that the movies will be generated
% according to statesRef.
statesByDist = statesRef;
...
% 1280/4*3 = 960; 2160/4*3 = 1620 FIG_HEIGHT = 960; FIG_WIDTH = 1280;
FIG_HEIGHT = 720;
FIG_WIDTH = 700;
...
for idxFrame = 2:numFrames
    ...
    % If the visible area is too large, or if a vehicle (except trucks)
    % left the visible area, ask for help.
    if exist('ASK_FOR_HELP', 'var')
        if(FLAG_SHOW_VEH_LABELS)
            ...
            for idxFileToShow = 1:length(filesToShow)
                ...
                if SHOW_VELOCITY_DIRECTIONS
                    ...
                    % value. currentSpeedToShow(idxFileToShow) =
                    % log(currentSpeedToShow(idxFileToShow)+1)/log(10);
            ...
            % Ask for help (human adjustment of the visible area) if
            % necessary.
            if(ASK_FOR_HELP.flag)
