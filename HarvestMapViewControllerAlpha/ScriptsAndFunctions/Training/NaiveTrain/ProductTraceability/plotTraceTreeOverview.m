...
% Line style (via patch) for edges.
edgeStyle = '--';
edgeColor = ones(1,3)*0.8;
edgeAlpha = 0.8;
% Marker style (via scatter) for different types of node.
markerStyles = containers.Map(layerLabels, ...
    {'o', '*', 'o', 'o', 'o', 'o'});
markerFaceColors = containers.Map(layerLabels, ...
    {'k', 'none', 'k', 'b', 'r', 'g'});
markerEdgeColors = containers.Map(layerLabels, ...
    {'w', 'k', 'w', 'w', 'w', 'w'});
markerLineWidths = containers.Map(layerLabels, ...
    {2, 1, 1, 1, 1, 0.1});
markerSizes = containers.Map(layerLabels, ...
    {60, 50, 30, 30, 30, 30});
markerFaceAlphas = containers.Map(layerLabels, ...
    {1, 0.5, 0.5, 0.5, 0.5, 0.5});
markerEdgeAlphas = containers.Map(layerLabels, ...
...
while ~all(isnan(indicesNodeToPlot))
    ...
    if SPACE_NODE_BY_PRODUCT_AMOUNT
        % Assign x spaces for its children according to their product
        % amounts.
...
hFig = figure; hold on;
% Edges.
hEdges = patch(xsEdge, ysEdge, edgeColor, ...
    'LineStyle', edgeStyle, 'EdgeColor', edgeColor, ...
    'EdgeAlpha', edgeAlpha);
...
for idxType = 1:length(layerLabels)
    ...
    if isempty(nodeLocsPerLayer{idxType})
        hNodeLayers{idxType} = nan;
    else
        hNodeLayers{idxType} ...
            = scatter(nodeLocsPerLayer{idxType}(:,1), ...
            nodeLocsPerLayer{idxType}(:,2), ...
            markerSizes(nodeTypeLabel), ...
            markerStyles(nodeTypeLabel), ...
            'MarkerFaceColor', markerFaceColors(nodeTypeLabel), ...
            'MarkerEdgeColor', markerEdgeColors(nodeTypeLabel), ...
            'LineWidth', markerLineWidths(nodeTypeLabel), ...
            'MarkerFaceAlpha', markerFaceAlphas(nodeTypeLabel), ...
            'MarkerEdgeAlpha', markerEdgeAlphas(nodeTypeLabel));
...
set(gca, 'XTick', []); grid minor; grid on;
